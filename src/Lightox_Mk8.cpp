...
//fixed from V6:
//10s LED warm up - addressed, but has a v.low start up light level (5/255 = 2%).
//Lid switch was too slow - improved
//Software version added to info screen
//Not yet fixed:
//Occasional skip on project squiggle
//Screen upside down - there does not seem to be an easy fix FTImpl.Cmd_Rotate(32768) seems to apply to bitmaps not the screen?
//Null files?
...
OneWire oneWire(53);  // Data wire is plugged into pin 53 on the Arduino pulled up with 4.7k resistor
...
char imagename[11][12] = {"Lightox.jpg","Run.jpg","Run.jpg","Run.jpg","Run.jpg","Run.jpg","Run.jpg","Sam.jpg","Carrie.jpg","Run.jpg","Run.jpg"};
int Screen = 0;   //Screen =   0             1          2            3          4          5          6
//char imagename[7][12] = {"Lightox.jpg","Menu.jpg","Date.jpg","Project.jpg","TIE.jpg","Sam.jpg","Carrie.jpg"};
...
char tempPrint[15] = {'t','e','m','p',' ','=',' ','0','0','0','.','0','\0'};
char OutputValue[5];
char TimeString[6] = "00:00";
//int TimeStepUp = 1, TimeStepUpCount = 0, TimeStepDown = 1, TimeStepDownCount = 0;
...
int TimeDigit = 2;
char ProjectString[150] = {'P','r','o','j','e','c','t',' ','d','e','s','c','r','i','p','t','i','o','n',' ','i','n',' ','u','p',' ','t','o',' ','4','0',' ','c','h','a','r','s','\0'};
char LogFile[20] = {'/','L','O','G','S','/','L','O','G','0','0','0','0','0','.','C','S','V'};
...
void setup(void)
{
  ...
  Energy =  (float)Time * (float)EnergyDensity / 1000.0 * (float)Intensity / 100.0;
...
void loop()
{
  ...
  int32_t tmpduration, tmpNewEnergyDensity, tmpNewCurrent = 100;    //, tmpintensity = 100
  char strduration[15], strNewEnergyDensity[15], strEnergy[15];   //, strintensity[15], strNewCurrent[15]
  char TmpDate[3];
  //end sliders
  ...
  char FlashFile[27]= {'$','W','R','I','T','E',' ','L','O','G','0','0','0','0','0','.','C','S','V'};
  bool DateRead = false;
  ...
  while(1)
  {   
    FTImpl.Cmd_DLStart();  //start new display list - ends with DL_swap///////////////////////////////////////////////////////////////////////////////////////
    ...
    if (Screen != 0) //On all other screen bottom left button is quit///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    {
    ...
    FTImpl.Tag(16);
    FTImpl.Cmd_Button(17,10,431,33,26,tagoption,"Copy logs to flash drive");

    tagoption = 0;//no touch is default 3d effect and touch is flat effect
    if(18 == tagval) tagoption = FT_OPT_FLAT;
    FTImpl.Tag(18);
    FTImpl.Cmd_Button(17,98,431,33,26,tagoption,"Settings (Administrator password required)");
    ...
    //FTImpl.Cmd_Text(60, 20, 26, FT_OPT_CENTER, "Duration");
      FTImpl.Cmd_Text(250, 25, 16, FT_OPT_CENTER, "Day    Month     Year     Hour   Minute  Second");

      if(tagval == 31)
      {
        TimeDigit = datetag-25;
        TimeAndDate[TimePointer[TimeDigit]] += 1;
        if (TimeAndDate[TimePointer[TimeDigit]] > TimeMax[TimeDigit]) TimeAndDate[TimePointer[TimeDigit]] = TimeMin[TimeDigit];
        delay(200);
      }
      ...
      if(tagval == 32)
      {
        TimeDigit = datetag-25;
        TimeAndDate[TimePointer[TimeDigit]] -= 1;
        if (TimeAndDate[TimePointer[TimeDigit]] < TimeMin[TimeDigit]) TimeAndDate[TimePointer[TimeDigit]] = TimeMax[TimeDigit];
        delay(200);
      }   
    }
 

    if (Screen == 4)   //Time Intensity Energy///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    {     
    ...
    tmpduration += (int32_t)durationS*59/65535;  //value is 0 - 3659 seconds      
    Time = tmpduration;
    ...
    if (Screen == 5)   //Current & power density///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    {
    ...
    tmpNewCurrent = (float)NewCurrent*100/65535+0.5; //value is 0 - 100%  
    Float = (float)NewEnergyDensity*300/65535+100;  //Value is 100 - 400 uW/mm2
    ...
    if (Screen == 6)   //Run  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    {
           ...
           Serial.println(SetCurrent);         
            ...
            if ((LogFile2 = sd.open(LogFile,FILE_WRITE)) == NULL) {            //Open and ensure file created o.k.
              Serial.print(F("File error"));
            }
            LogFile2.println("Deliberately left blank");                       //Sometimes lose first line, so make it a dummy.
            ...
            analogWrite(ADM,0);
            analogWrite(PWM,5);                         //LEDs power setting - see Mk6 - starting with PWM = 0 causes LED's to fail.
            delay(100);
            digitalWrite(FAN,HIGH);
            delay(1000);                                                        //to make sure current setting voltage R-C circuit is charged 
            ...
            do
            {
              ...
              TimeString[4] = char(48 + iTime - 60 * int(iTime / 60) - 10 * int((iTime - 60 * int(iTime / 60))/10));
              ...
              {
                LidOpen = true;
                msTimeLid = millis();                       //need to keep a record of how long lid is open for
                analogWrite(PWM,0);
                //Serial.println("Close Lid");
                //analogWrite(ADM,0);
                //analogWrite(PWM,int((float)Intensity*2.55));              //reset start case
              }
              if(LidOpen)
              {
              ...
              if(OldiTime != iTime) //into a new second, so save results
              {
                ...
                LogFile2.print(", "); 
                uvValue = uv.readUV();
                sprintf (uvPrintVal, "%05i", uvValue);  //%0 left pads the number with zeros, 5 = width, i = signed decimal integer
                for (i=0;i<5;i++)
                {
                ...
                //Serial.println(i);
                delay(10);
                sensors.requestTemperaturesByAddress(DallasAddress);    //can take up to 750ms to get temperature?
                Temperature = sensors.getTempC(DallasAddress);
                if (Temperature != -127) 
                {
                  Serial.println(Temperature);
                  sprintf (tempPrintValA, "%03i", int(Temperature));
                  sprintf (tempPrintValB, "%01i", int((Temperature - int(Temperature))*10));   
                  for (it=0;it<3;it++)
                  {
...
EscapeNestedLoops:            
    ...
    if (Screen == 10)   //Info
    {
      ...
      FTImpl.Cmd_Text(230,60,26,FT_OPT_CENTER,"405nm Luminus SST-10-UV LEDs");
      FTImpl.Cmd_Text(230,90,26,FT_OPT_CENTER,"ST LED6001 controller");
      FTImpl.Cmd_Text(230,120,26,FT_OPT_CENTER,"Drive current: 100% = 0.5A, 0% = OFF");      
      FTImpl.Cmd_Text(230,150,26,FT_OPT_CENTER,"Software version 8.0");
      FTImpl.Cmd_Text(230,200,26,FT_OPT_CENTER,"Designed & built by BNC for Lightox");       
    }
    
    if(sd_present)
    {
      FTImpl.Cmd_Text(40,FT_DISPLAYHEIGHT/3,28,FT_OPT_CENTERY,"Storage Device not Found");
    } 
    ...
    if (tagval == 13) //quit
    {
      FTImpl.Cmd_DLStart();        
      FTImpl.ClearColorRGB(64,64,64);
      FTImpl.Clear(1,1,1);FTImpl.ColorRGB(0xff, 0xff, 0xff); 
      FTImpl.Display();
      FTImpl.Cmd_Swap();
      FTImpl.Finish();
      Screen = 0;
    ...
    if (tagval == 14) //Run / Save.............run option not coded yet
    {
      ...
      else if (Screen == 2) //=Date
      {
        ...
        SetTimeDate(TimeAndDate[TimePointer[0]],TimeAndDate[TimePointer[1]],TimeAndDate[TimePointer[2]],TimeAndDate[TimePointer[3]],TimeAndDate[TimePointer[4]],TimeAndDate[TimePointer[5]]); 
        Screen = 0;
        Loadimage2ram();
        DateRead = false;
      }
    ...
    if (tagval == 16) //Copy files
    {
      ...
      //{
      //  i++;
      //}while((!mySerial.available()) && (i < 32000));        // Wait for data to be returned
      ...
      {  
              ...
              {
                LogRef++;
                LogFile[11] = char(48+int(LogRef/100));                       //can cope with up to 999 files
                LogFile[12] = char(48+int((LogRef-int(LogRef/100)*100)/10));
                LogFile[13] = char(48+int(LogRef-int(LogRef/10)*10));           
                Serial.println(LogFile);    
                if (sd.exists(LogFile)) 
                {
                  ...
                  else
                  {
                    ...
                    else
                    {
                      ...
                      for (i=0;i<80;i++) sdlogbuffer[i] = '\0';    //clear the log buffer
                      while(LogFile2.available() > 0)
                      {
                        ...
                        else
                        {
                          ...
                          LineCountString[0] = char(48+int(LineCount/1000));   //can cope with up to 9999 lines
                          LineCountString[1] = char(48+int((LineCount-int(LineCount/1000)*1000)/100));                      
                          LineCountString[2] = char(48+int((LineCount-int(LineCount/100)*100)/10));
                          LineCountString[3] = char(48+int(LineCount-int(LineCount/10)*10));  
                          LineCountString[4] = '\0';
                          FTImpl.Cmd_Text(230, 160, 29, FT_OPT_CENTER, "Line:");                 
                          FTImpl.Cmd_Text(230, 190, 29, FT_OPT_CENTER, LineCountString);
                ...
                FTImpl.BitmapSize(FT_NEAREST,FT_BORDER,FT_BORDER,480,272);           
                FTImpl.ColorRGB(0xff, 0xff, 0xff);  
                LineCountString[0] = char(48+int(logcopycount/100));                       //can cope with up to 999 lines
                LineCountString[1] = char(48+int((logcopycount-int(logcopycount/100)*100)/10));
                LineCountString[2] = char(48+int(logcopycount-int(logcopycount/10)*10));       
                LineCountString[3] = '\0';      
...
{  
  ...
  FTImpl.Clear(1,1,1);    
  FTImpl.ColorRGB(0xff, 0xff, 0xff);    
  FTImpl.Cmd_Text((FT_DISPLAYWIDTH/2), (FT_DISPLAYHEIGHT/2)-40, 27, FT_OPT_CENTER, "Power-up screen calibration.");
  FTImpl.Cmd_Text((FT_DISPLAYWIDTH/2), (FT_DISPLAYHEIGHT/2), 27, FT_OPT_CENTER, "Please tap carefully on the dot.");
  //FTImpl.Cmd_Text((FT_DISPLAYWIDTH/2), (FT_DISPLAYHEIGHT/2)+40, 27, FT_OPT_CENTER, "with a pointer");
...
void Load_Jpeg(FT_SDFile &fname) //&fname appears to be the address of the Imagefile
{
  uint8_t imbuff[512];
...
void Loadimage2ram()
{
  ...
  FTImpl.WriteCmd(0);  //pointer - was 500: 100  262144L instead of 500 gives blank pages
  FTImpl.WriteCmd(0); 
  Load_Jpeg(Imagefile);
  return;
}
...
void UpdateTime(uint16_t color)
{
  ...
  TimeString[4] = char(48 + Time - 60 * int(Time / 60) - 10 * int((Time - 60 * int(Time / 60))/10));
...
{
  char *temp;                                           //The "*" makes it an indirection operator, which means it points to the address of a variable, so temp = x, points to the address of x.
...
//Clear then set Linebuffer
  ...
  memset(&Buffer.notepad[tval],'\0',sizeof(Buffer.notepad[tval]));  //set all of buffer to be null
  noofchars = 0;

  if(Screen == 3) //Run
  {
    for(i=0;i<strlen(ProjectString);i++)  //load in the Project Description
    {
...
/*intial setup*/
  ...
  memset((Buffer.notepad[Line]+noofchars),'_',1);                         // For Cursor //noofchars was 0
...
/*enter*/
  ...
  if(21 == Read_sfk)  //save
  {
      ...
      {
        if((Buffer.notepad[0][0] == '3') &&  (Buffer.notepad[0][1] == '3') &&(Buffer.notepad[0][2] == '3') &&(Buffer.notepad[0][3] == '3')) 
        {
  ...
  // String and Line Termination    
      ...
      if(strlen(Buffer.notepad[0]) < 40)                                //attempt to limit the character count
      {
...
// Start the new Display list
  ...
  FTImpl.Cmd_Button((FT_DISPLAYWIDTH*0.855),(FT_DISPLAYHEIGHT*0.83-offset),(FT_DISPLAYWIDTH*0.146),(FT_DISPLAYHEIGHT*0.112),font,But_opt,"Clear");  
  But_opt = (Read_sfk==BACK_SPACE)? FT_OPT_FLAT:0;
  FTImpl.Tag(BACK_SPACE);                         // BackSpace
  FTImpl.Cmd_Button((FT_DISPLAYWIDTH*0.875),(FT_DISPLAYHEIGHT*0.70-offset),(FT_DISPLAYWIDTH*0.125),(FT_DISPLAYHEIGHT*0.112),font,But_opt,"<-"); 
  But_opt = (Read_sfk==' ')? FT_OPT_FLAT:0;
  FTImpl.Tag(' ');                              // Space
  FTImpl.Cmd_Button((FT_DISPLAYWIDTH*0.115),(FT_DISPLAYHEIGHT*0.83-offset),(FT_DISPLAYWIDTH*0.73),(FT_DISPLAYHEIGHT*0.112),font,But_opt,"Space");

  if(Flag.Numeric==OFF)
  {
    FTImpl.Cmd_Keys(0,(FT_DISPLAYHEIGHT*0.442-offset),FT_DISPLAYWIDTH,(FT_DISPLAYHEIGHT*0.112),font,Read_sfk,(Flag.Caps == ON ?"QWERTYUIOP":"qwertyuiop"));
    FTImpl.Cmd_Keys((FT_DISPLAYWIDTH*0.042),(FT_DISPLAYHEIGHT*0.57-offset),(FT_DISPLAYWIDTH*0.96),(FT_DISPLAYHEIGHT*0.112),font,Read_sfk,(Flag.Caps == ON ?"ASDFGHJKL":"asdfghjkl"));
    FTImpl.Cmd_Keys((FT_DISPLAYWIDTH*0.125),(FT_DISPLAYHEIGHT*0.70-offset),(FT_DISPLAYWIDTH*0.73),(FT_DISPLAYHEIGHT*0.112),font,Read_sfk,(Flag.Caps == ON ?"ZXCVBNM":"zxcvbnm")); 

    But_opt = (Read_sfk== CAPS_LOCK)? FT_OPT_FLAT:0;
    FTImpl.Tag(CAPS_LOCK);                                  // Capslock
    FTImpl.Cmd_Button(0,(FT_DISPLAYHEIGHT*0.70-offset),(FT_DISPLAYWIDTH*0.10),(FT_DISPLAYHEIGHT*0.112),font,But_opt,"a^");
    But_opt = (Read_sfk== NUMBER_LOCK)? FT_OPT_FLAT:0;
    FTImpl.Tag(NUMBER_LOCK);                                // Numberlock   
    FTImpl.Cmd_Button(0,(FT_DISPLAYHEIGHT*0.83-offset),(FT_DISPLAYWIDTH*0.10),(FT_DISPLAYHEIGHT*0.112),font,But_opt,"12*");
  }
  if(Flag.Numeric==ON)
  {   
    FTImpl.Cmd_Keys(0,(FT_DISPLAYHEIGHT*0.442-offset),FT_DISPLAYWIDTH,(FT_DISPLAYHEIGHT*0.112),font,Read_sfk,"1234567890");
    FTImpl.Cmd_Keys((FT_DISPLAYWIDTH*0.042),(FT_DISPLAYHEIGHT*0.57-offset),(FT_DISPLAYWIDTH*0.96),(FT_DISPLAYHEIGHT*0.112),font,Read_sfk,"-@#$%^&*(");
    FTImpl.Cmd_Keys((FT_DISPLAYWIDTH*0.125),(FT_DISPLAYHEIGHT*0.70-offset),(FT_DISPLAYWIDTH*0.73),(FT_DISPLAYHEIGHT*0.112),font,Read_sfk,")_+[]{}");        
    But_opt = (Read_sfk== NUMBER_LOCK)? FT_OPT_FLAT:0;
    FTImpl.Tag(253);                          // Numberlock
    FTImpl.Cmd_Button(0,(FT_DISPLAYHEIGHT*0.83-offset),(FT_DISPLAYWIDTH*0.10),(FT_DISPLAYHEIGHT*0.112),font,But_opt,"AB*");
  }
  ...
  FTImpl.ScissorSize(FT_DISPLAYWIDTH,(uint16_t)(FT_DISPLAYHEIGHT*0.405-offset));  
